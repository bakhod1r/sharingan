...
extension Bundle {
    /// The `Sharingan` target's resource bundle, resolved from a code-signable location.
    ///
    ...
    static let sharinganAppResources: Bundle = {
        if let res = Bundle.main.resourceURL,
           let bundle = Bundle(url: res.appendingPathComponent("Sharingan_Sharingan.bundle")) {
            return bundle
        }
        return .module
    }()
