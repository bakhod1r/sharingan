...
// Headless preview of the Report tab's per-task app breakdown (debug utility):
// `Sharingan --render-report-apps <path>`. Draws sample task rows each with a
// TaskAppStrip beneath — the SAME component the real expanded Report row uses,
// so this photographs what ships. Touches no store: it only draws a view (the
// bundle IDs resolve to real icons via NSWorkspace on this Mac).
if let i = CommandLine.arguments.firstIndex(of: "--render-report-apps"),
   i + 1 < CommandLine.arguments.count {
    let out = CommandLine.arguments[i + 1]
    MainActor.assumeIsolated {
        struct SampleRow: View {
            var code: String, title: String, category: String, categoryColor: Color
            var count: Int, seconds: TimeInterval
            var apps: [AnalyticsEngine.AppTotal]
            var body: some View {
                VStack(spacing: 2) {
                    HStack(spacing: 10) {
                        ...
                        Text(category.uppercased())
                            ...
                            .tracking(0.6).foregroundStyle(categoryColor)
                        ...
                        Text(AnalyticsAppsView.durationLabel(seconds))
                    ...
                    TaskAppStrip(apps: apps)
                }
            }
        }
        func t(_ b: String, _ s: TimeInterval) -> AnalyticsEngine.AppTotal {
            AnalyticsEngine.AppTotal(bundleID: b, seconds: s)
        }
        let preview = VStack(alignment: .leading, spacing: 8) {
            Text("Thu, Jul 16 · per-task app focus")
                .font(.system(.headline, design: .rounded).weight(.semibold))
                .foregroundStyle(.white)
            SampleRow(code: "T-4", title: "API Implementation: Session Management",
                      category: "Work", categoryColor: Color(red: 0.36, green: 0.55, blue: 1.0),
                      count: 3, seconds: 4500,
                      apps: [t("com.microsoft.VSCode", 2700), t("com.google.Chrome", 1200),
                             t("com.apple.Terminal", 600)])
            SampleRow(code: "T-5", title: "Leetcode Stack",
                      category: "Study", categoryColor: Color(red: 0.55, green: 0.78, blue: 0.42),
                      count: 2, seconds: 3000,
                      apps: [t("com.apple.dt.Xcode", 2100), t("com.apple.Safari", 900)])
        }
        .padding(20)
