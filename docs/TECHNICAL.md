...
## Jira Cloud integration (since 1.7.0)

Manage Jira work from inside Sharingan without opening Jira. All net-new code lives in `Sources/SharinganCore/Services/Jira/` (mirrors the `Sync/` layout); UI in `Sources/Sharingan/Views/Jira/`. Off until connected; the app is otherwise unchanged.

- **Auth — OAuth 2.0 (3LO).** "Log in with Atlassian" (`JiraOAuth`): authorize URL → a one-shot `NWListener` on the registered loopback callback (`http://localhost:53682/callback`, `state`-checked for CSRF) → code exchange → `accessible-resources` for the cloud id. Atlassian doesn't support PKCE and requires a `client_secret`, so the app ships one, XOR-masked and baked into Info.plist by `make-app.sh` from the environment (`JIRA_CLIENT_ID`/`JIRA_CLIENT_SECRET`; repo secrets in CI). The mask only keeps it out of `strings` and secret scanners — it is **not** a security boundary (a debugger or TLS proxy recovers it); `JiraAppCredentials.swift` says so. A backend broker is the real fix and is deferred. **Every REST call goes through the gateway** `https://api.atlassian.com/ex/jira/{cloudId}/…` with a `Bearer` token; refresh tokens rotate, so token vending is single-flight behind an actor (two concurrent refreshes would invalidate each other and silently log the user out). Tokens live in the Keychain (`JiraTokenStore`); only non-secret bits (expiry, cloudId, site, scopes) go to UserDefaults. A `mypermissions` preflight at connect distinguishes "no issues" from "can't see any projects" — `/search/jql` returns 200 + empty for a project you can't browse, so without it the user sees "Connected" next to a silently empty list.
- **Import (pull) — hierarchy-aware.** `syncAssignedIssues` collects all pages (`POST /rest/api/3/search/jql`; the legacy `/search` is gone, and it returns no `total`), then `JiraFieldMapper.buildHierarchy` nests sub-tasks (decided by the issue type's `subtask` flag, never `parent` presence — Stories carry their Epic as `parent` and must stay top-level) under their parents. Orphan sub-tasks (parent assigned to someone else) trigger one batched `key in (…)` query to fetch the missing parents. A Jira sub-task becomes a Sharingan `Subtask` carrying its own `jiraKey`/`jiraIssueID` (so worklogs/transitions can target it); Epic/Story/Bug/Task become top-level tasks with `TaskItem.jiraIssueType` for a colour-coded badge. Earlier flat imports of a sub-task self-heal: their pomodoro progress transfers into the nested subtask and the flat row is deleted. Field mapping (`JiraFieldMapper`, pure, exhaustively tested): priority ↔ `TaskPriority`, labels ↔ tags (components held out of the push so they aren't re-pushed as labels), project → category, due date, estimate (`ceil(sec/1500)` pomodoros). Import is scoped to a chosen project ("space").
- **Two-way sync (opt-in, `jira.syncMode`).** Pull is the default. In two-way mode a local edit to a linked task (`TaskStore.jiraChangeObserver` → `JiraService.taskDidChange`) diffs against the last-seen snapshot (`JiraFieldMapper.pushFields`) and queues a `fields` op, coalescing repeated edits to one issue into a single row. A sync sets `isApplyingRemote` so Jira's own values coming back aren't recaptured (no bounce-back). The **outbox** (`jira_outbox` in `JiraStorage`, never CloudKit-synced) is drained by `JiraOutboxFlusher`: `fields` → PUT, `worklog` → POST; transient errors back off (30s → 2m → 10m → 1h → hourly), permanent ones (4xx) mark the row failed for a Settings "couldn't be sent" notice. `pushNow()` and the poll drain it; the poll (`jira.pollMinutes`) pulls then pushes.
- **Status & board.** Each linked task row shows a status chip (`JiraStatusChip`, cached status, tinted by category); tapping fetches the workflow transitions (on menu-open, not per visible row) and applies one immediately (interactive, not queued), updating the cache. The **sprint board** (`JiraBoardModel`/`JiraBoardView`, Agile API) shows only your cards; a card maps to a column by status **id** (a custom workflow renames statuses, so names can't be trusted; unmapped → an "Other" column), and dragging it between columns transitions it optimistically — picking the transition whose target status id is in the destination column, snapping back on failure or a `hasScreen` transition (needs a Jira field screen we can't render). The board lives in the **Board** section's **Jira** tab (`BoardSectionView`, since 1.8.0 — it was a sheet opened from Tasks before); the Tasks view-bar button now deep-links there (`AppRouter.openBoard(tab: .jira)`), and while Jira is disconnected the tab shows a connect-in-Settings hint instead. The board model is created once per window on first visit, so switching tabs doesn't refetch. A **Switch board** button (`forgetBoard()` + reload) re-shows the picker for a multi-board project. Optionally (Settings → Integrations, `jira.addToActiveSprint`), converting a task to Jira also drops the new issue into the project's active sprint — best-effort via `JiraClient.addIssuesToSprint`, non-fatal when there's no active sprint.
- **Pomodoro → worklog.** `SharinganCoordinator` reports a completed focus via `JiraPomodoroHooks` (Core can't see the app-target `JiraService`); in two-way mode with worklog sync on, it queues a worklog against the active sub-task's issue when linked, else the parent's. Same non-mirrored guard as the pomodoro credit, so only the session's owner Mac logs — no cross-Mac double-post. `started = completion − seconds` in Jira's POSIX format (ISO8601's colon-offset is rejected), sub-minute sessions dropped, `adjustEstimate=auto` lets Jira reduce the remaining estimate.
- **Issue detail** (`JiraIssueDetailModel`/`View`, from the row's "…" menu): description, comments, full change history, worklogs — each an independent loading/loaded/failed section so "no comments yet" reads differently from a failed fetch. Post comments from here. Rich text (ADF) renders read-only as plain text via `ADF.plainText`; edits save as plain-text ADF, with a caption saying so.
- **Create (Sharingan → Jira).** A task's "…" menu → "Create Jira issue" files it in the mapped project (`jira.categoryProjectMap`, edited in Settings) and links the returned key back. `JiraClient.createIssue` takes `JiraIssueCreateFields` (project + issuetype are required on create).
- **Cross-Mac.** The three link fields plus `jiraIssueType` ride CloudKit on the `Task` record (subtask keys ride the existing `subtasksJSON`); the token, issue cache and outbox are per-Mac and never sync. **Release gate: promote the five new `Task` fields (`jiraKey`, `jiraIssueID`, `jiraSiteHost`, `jiraIssueType`, and `boardColumnID` for the custom board) dev→production in the CloudKit dashboard before shipping.**
