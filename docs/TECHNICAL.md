...
- **Cross-Mac.** The three link fields plus `jiraIssueType` ride CloudKit on the `Task` record (subtask keys ride the existing `subtasksJSON`); the token, issue cache and outbox are per-Mac and never sync. **Release gate: promote the four new `Task` fields (`jiraKey`, `jiraIssueID`, `jiraSiteHost`, `jiraIssueType`) dev→production in the CloudKit dashboard before shipping.**
