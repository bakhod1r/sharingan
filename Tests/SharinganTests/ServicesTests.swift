...
struct BreakExerciseTests {
    ...
    @Test @MainActor func strictModeNeverAdvancesUnseenGazeStep() {
        let v = ExerciseValidator()
        v.strictValidation = true
        v.exercises = [BreakExercise(name: "test", steps: [
            .init(direction: "left", holdSeconds: 1),
        ])]
        v.start()
        defer { v.stop() }
        // Camera never sees the movement — drive time far past hold + grace.
        for _ in 0..<200 { v.tick(0.1) }
        #expect(v.currentStepIndex == 0)
        #expect(v.exercisesCompleted == 0)
    }

    @Test @MainActor func strictModeAdvancesOnceCameraConfirms() {
        let v = ExerciseValidator()
        v.strictValidation = true
        v.exercises = [BreakExercise(name: "test", steps: [
            .init(direction: "left", holdSeconds: 1),
            .init(direction: "right", holdSeconds: 1),
        ])]
        v.start()
        defer { v.stop() }
        v.ingest(gaze: .left, isBlinking: false)
        for _ in 0..<12 { v.tick(0.1) }
        #expect(v.currentStepIndex == 1)
    }

    @Test @MainActor func strictModeStillAdvancesGuidanceOnlySteps() {
        let v = ExerciseValidator()
        v.strictValidation = true
        v.exercises = [BreakExercise(name: "test", steps: [
            .init(direction: "far", holdSeconds: 1),
            .init(direction: "closed", holdSeconds: 1),
        ])]
        v.start()
        defer { v.stop() }
        // "far"/"closed" can't be camera-validated — no ingest at all, they
        // must still complete on their hold timers even in strict mode.
        for _ in 0..<12 { v.tick(0.1) }
        #expect(v.currentStepIndex == 1)
    }

    @Test @MainActor func nonStrictGraceStillAdvancesUnmatchedStep() {
        let v = ExerciseValidator()
        v.exercises = [BreakExercise(name: "test", steps: [
            .init(direction: "left", holdSeconds: 1),
            .init(direction: "right", holdSeconds: 1),
        ...
        v.start()
        defer { v.stop() }
        v.ingest(gaze: .right, isBlinking: false)  // mismatch → retry flagged
        for _ in 0..<70 { v.tick(0.1) }            // past hold + 5s grace
        #expect(v.currentStepIndex == 1)
    }
}
